...
describe('POST /auth/signup', () => {
  ...
  test('User Registration with mismacth password and confirm password',
      async () => {
        const createUser = {
          name: 'Test Example',
  ...
  test('Verify User With Incorrect OTP', async () => {
    const id = (await User.find({email: 'dipronildas.net@gmail.com'}, '_id')
        .exec())[0];
    const verifyOtp = {
      otp: 123456,
    };
    const response = await request(app)
        .put('/auth/verifyotp/'+ id._id)
        .send(verifyOtp);
    const jsonResponse = JSON.parse(response.text);
    expect(jsonResponse.status).toEqual(false);
